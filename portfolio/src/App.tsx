...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    document.documentElement.style.setProperty('--darkness-level', darkness.toString());

    // Add body classes based on scroll depth
    const body = document.body;
    body.classList.remove('surface', 'shadows', 'twin-peaks', 'chavon', 'artistic', 'movies', 'void');
  ...
  return (
    <div className="App">
      ...
      {/* Lynch-inspired geometric elements that react to scroll */}
      <div className="geometric-overlay">
        <div
          className="floating-triangle"
          style={{
            transform: `rotate(${scrollProgress * 360}deg) scale(${1 + scrollProgress})`,
        ...
        <div
          ...
          style={{
            transform: `scale(${Math.sin(scrollProgress * Math.PI)})`,
            filter: `hue-rotate(${scrollProgress * 180}deg)`
          }}
        />
      </div>
